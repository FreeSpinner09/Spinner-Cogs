...
class McSync(commands.Cog):
    ...
    async def _test_rcon(self, host: str, port: int, password: str, retries: int = 3, timeout: int = 15):
        """Try connecting to RCON with given details, with retries and detailed error logging."""
        async with self.rcon_lock:
            for attempt in range(1, retries + 1):
                try:
                    ...
                    logger.error(f"RCON connection refused for {host}:{port} on attempt {attempt}: {e}")
                    if attempt == retries:
                        return f"RCON connection refused: {e}"
                except socket.timeout as e:
                    logger.error(f"RCON timeout for {host}:{port} on attempt {attempt}: {e}")
                    if attempt == retries:
                        return f"RCON timeout: {e}"
                except (ConnectionResetError, BrokenPipeError) as e:
                    logger.error(f"RCON connection reset for {host}:{port} on attempt {attempt}: {e}")
                    if attempt == retries:
    ...
    async def _send_rcon(self, guild: discord.Guild, command: str, retries: int = 3, timeout: int = 15) -> str:
        """Send a command via RCON with this guild’s config, with retries and detailed error logging."""
        ...
        async with self.rcon_lock:
            for attempt in range(1, retries + 1):
                try:
                    ...
                    logger.error(f"RCON connection refused for command '{command}' on attempt {attempt}: {e}")
                    if attempt == retries:
                        return f"⚠️ RCON connection refused: {e}"
                except socket.timeout as e:
                    logger.error(f"RCON timeout for command '{command}' on attempt {attempt}: {e}")
                    if attempt == retries:
                        return f"⚠️ RCON timeout: {e}"
                except (ConnectionResetError, BrokenPipeError) as e:
                    logger.error(f"RCON connection reset for command '{command}' on attempt {attempt}: {e}")
                    if attempt == retries:
